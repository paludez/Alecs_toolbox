...
class ALEC_MT_quad_menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        # Pivot
        b_pivot = col_left.box()
        b_pivot.label(text="Pivot Point", icon='PIVOT_MEDIAN')
        
        ts = context.scene.tool_settings
        
        # columns=2 forțează așezarea în 3 rânduri (dacă avem 6 elemente)
        grid = b_pivot.grid_flow(columns=3, align=True, even_columns=True)
        
        # Primele 5 sunt punctele de pivot standard
        ...
        # Orientation
        b_orient = col_right.box() # Sau col_left, unde ai locul mai liber
        b_orient.label(text="Orientation", icon='ORIENTATION_GLOBAL')
        
        # Orientarea este stocată în "slots[0]" în versiunile noi de Blender
        slot = context.scene.transform_orientation_slots[0]

        grid = b_orient.grid_flow(columns=2, align=True, even_columns=True)
        ...
        # Snap Settings
        b5 = col_right.box()
        b5.prop(ts, "use_snap", text="Use Magnet", toggle=True)
        
        r5 = b5.row(align=True)
...
class ALEC_MT_menu_orientation(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        for value, label, icon in [('GLOBAL', "Global", 'ORIENTATION_GLOBAL'), ('LOCAL', "Local", 'ORIENTATION_LOCAL'), ('NORMAL', "Normal", 'ORIENTATION_NORMAL'), ('GIMBAL', "Gimbal", 'ORIENTATION_GIMBAL'), ('VIEW', "View", 'ORIENTATION_VIEW'), ('CURSOR', "Cursor", 'ORIENTATION_CURSOR'), ('PARENT', "Parent", 'ORIENTATION_PARENT')]:
